...
function removeSpecificDominoCard() {
    const leftInput = document.getElementById("remove-left-number");
    const rightInput = document.getElementById("remove-right-number");
    const leftNumber = Number(leftInput.value);
    const rightNumber = Number(rightInput.value);
    // Pastikan input valid
    if (!isNaN(leftNumber) && !isNaN(rightNumber)) {
        // Hapus kartu domino berdasarkan nilai di leftNumber dan rightNumber
        dominoData = dominoData.filter((card) => !(card[0] === leftNumber && card[1] === rightNumber));
...
function removeDuplicates() {
    ...
    const seen = new Set(); // Set untuk melacak kombinasi kartu yang sudah dilihat
